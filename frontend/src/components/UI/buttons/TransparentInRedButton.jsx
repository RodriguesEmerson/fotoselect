 ...
 */
export function TransparentInRedButton({ size = 'mid', children, ...props }) {
   const sizes = {
      small: 'min-w-20 w-20',
      mid: 'min-w-48 w-48',
      large: 'min-w-62 w-62',
   ...
   return (
      <button
         type="button"
         className={`flex items-center justify-center gap-2 h-10 px-4 rounded-md text-red-800 bg-[var(--backgroung)] border border-red-700 cursor-pointer hover:bg-red-50 transition-all ${sizes[size]}`}
         {...props}
      >
         {children}
      </button>
